const Constants = {
  Email: 'asraparigasdocodigo@gmail.com'
};

const Routes = {
  About: '/sobre',
  Blog: '/blog',
  Collaborate: '/colaborar',
  Contacts: '/contactos',
  Events: '/eventos',
  Home: '/',
  TermsOfUsePolicy: '/termsofuse',
  Mentorships: '/mentorias',
  PrivacyPolicy: '/privacy',
  Team: '/equipa',
  Workshops: '/workshops'
};

const SiteContent = {
  Title: {
    AboutUs: 'Sobre nós',
    AboutTheProject: 'Sobre o projecto',
    Blog: 'Blog',
    Collaborate: 'Colabora',
    Contacts: 'Contactos',
    DoYouWantToCollaborate: 'Queres colaborar connosco?',
    Events: 'Eventos',
    GetToKnowOurMentors: 'Conhece os nossos mentores.',
    Home: 'Home',
    IWantToCollaborate: 'Quero Colaborar!',
    TermsOfUsePolicy: 'Termos de utilização',
    MentorshipProgram: 'PROGRAMA DE MENTORIA',
    MentorshipForm: 'Procuras mentoria?',
    Mentorship: 'Mentoria',
    Mentorships: 'Mentorias',
    OurTeam: 'A nossa equipa',
    PrivacyPolicy: 'Política de Privacidade',
    WorkshopsList: 'Os nossos workshops',
    WorkshopsListNext: 'Próximos workshops',
    RdC: 'As Raparigas do Código',
    WeAreRdC: 'Somos As Raparigas do Código',
    WhatDoWeDo: 'O que fazemos?',
    Workshops: 'Workshops'
  },
  Subtitle: {
    AboutUs: 'TBD',
    Blog: 'TBD',
    Collaborate: 'TBD',
    Contacts: 'Não hesites em contactar-nos para qualquer informacão',
    GetToKnowOurMentors: 'TBD',
    Events: 'TBD',
    Mentorship: 'Sentes-te perdid@? Precisas de apoio mas não sabes a quem pedir?',
    MentorshipForm: 'Preenche este formulário e iremos responder-te o mais breve possível.',
    OurTeam: 'Conhece a nossa equipa!',
    Workshops:
      'Oferecemos vários cursos e workshops de programação, totalmente gratuitos e adaptados às necessidades e interesses das alunas inscritas.',
    WorkshopsList:
      'Podes ver todos os workshops que já realizámos e aqueles que ainda estarão por acontecer!',
    WorkshopsListNext: 'Reserva já o teu lugar!',
    WorkshopsListOld: 'Visita os workshops que já realizámos'
  },
  Text: {
    Copyright: `Copyright © ${new Date().getFullYear()} As Raparigas do Código. All Rights Reserved`,
    WhoWeAre:
      'Uma organização focada em desmistificar o papel da mulher na tecnologia, promovendo a inclusão digital e encorajando mais raparigas e mulheres a ingressar no sector das tecnologias de informação.',
    OurMission_p1:
      'Garantir que todas as mulheres que queiram aprender sobre tecnologia tenham um espaço onde o possam fazer rodeadas de colegas e pares que as representam e apoiam.',
    OurMission_p2:
      'Queremos desmistificar o papel da mulher na tecnologia, encorajando mais raparigas e mulheres a ingressar no sector das tecnologias de informação.'
  },
  Empty: {
    WorkshopsList: 'Infelizmente, não temos workshops, mas aceitamos sugestões. Contacta-nos!',
    WorkshopsListNext: 'Aguarda pelos próximos workshops! Se tiveres sugestões, contacta-nos!'
  },
  Buttons: {
    WorkshopCard: 'Quero inscrever-me !'
  }
};

export { Constants, Routes, SiteContent };
